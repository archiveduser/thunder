...
target_path=/opt/thunder/pan-xunlei-com/target
...
target_var=/opt/thunder/pan-xunlei-com/target/var
if [ ! -d "$target_var" ]; then
    path="/opt/thunder/pan-xunlei-com/target/host/etc"
    mkdir -p $path
    id=$(cat /proc/sys/kernel/random/uuid | cut -c1-7)
    echo "unique=\"synology_${id}_720+\"" >$path/synoinfo.conf
    chmod 755 $path/synoinfo.conf

    path="/opt/thunder/pan-xunlei-com/target/host/usr/syno/synoman/webman/modules"
    mkdir -p $path
...
ln -s /opt/thunder/pan-xunlei-com/target/host/etc/synoinfo.conf /etc/synoinfo.conf
ln -s /opt/thunder/pan-xunlei-com/target/host/usr/syno/synoman/webman/modules/authenticate.cgi /usr/syno/synoman/webman/modules/authenticate.cgi
